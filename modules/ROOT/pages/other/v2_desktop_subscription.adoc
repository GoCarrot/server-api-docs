...
== Category Update
:endpoint: v2/desktop_push/subscription_status/:category_identifier
[cols="1,3a", stripes="even"]
|===
| Endpoint
| https://api.gocarrot.com/{endpoint}
...
Description: The POST `{endpoint}` endpoint updates the desktop web push subscription status for a given player and Opt Out Category.
