...
Setting up the client side — starting a Live Activity, registering its push token, and choosing its id — is covered in the SDK guides: xref:ios::live-activities.adoc[iOS Live Activities] and xref:unity::live-activities.adoc[Unity Live Activities].
...
| live_activity_id
| The identifier your app assigns to a Live Activity when it starts one and registers its push token with Teak (the SDK calls this the `activityId`, e.g. `chest_timer_0`). It must be unique per concurrent instance, and Teak keys both scheduling and cancellation off it. See the xref:ios::live-activities.adoc[SDK guides] for how it is chosen and registered.

| custom_data
| The Live Activity's updated https://developer.apple.com/documentation/activitykit/activityattributes/contentstate[ContentState, window=_blank] — the game-defined values iOS renders into the Live Activity UI. Teak delivers it verbatim as the APNs `aps` `content-state`, so its shape must match your app's ActivityKit `ContentState`.

| system_data
| Optional APNs system fields for the update. Only four keys are honored — `event`, `stale-date`, `dismissal-date`, and `alert` — and any other key is ignored. `event` defaults to `update`; set it to `end` to end the Live Activity. Including an `alert` makes the update user-visible (an alerting push) instead of a silent content update.
|===
...
| custom_data
| A map of Game Assigned Player IDs to that player's `custom_data` (the Live Activity `content-state` — see Concepts above). Each value may be either a JSON object or a pre-serialized JSON string. A player is only scheduled if they have an entry here; any player in `user_ids` omitted from `custom_data` is reported in `errors.user_ids` and not scheduled.
| system_data
| A map of Game Assigned Player IDs to that player's `system_data` (the honored APNs system fields — see Concepts above). Each value may be either a JSON object or a pre-serialized JSON string. Players may be omitted.
| device_ids
...
NOTE: A `send_time` more than 30 days in the future is rejected with a `422`; separately, each update must fall within 10 hours of when its player's Live Activity was created, and any player outside that window is reported in `errors.user_ids` while the rest of the batch is scheduled.
