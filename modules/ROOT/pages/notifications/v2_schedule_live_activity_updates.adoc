...
| send_time
| The time in UTC at which to deliver the update. Must be in ISO8601 format, e.g. "2018-08-23 17:00:00". If omitted, defaults to the current time and delivers as soon as possible. Must be within 10 hours of when the player's Live Activity was created; a player whose `send_time` falls outside that window is reported in `errors.user_ids` and not scheduled.
| custom_data
| A map of Game Assigned Player IDs to the update payload for that player. Each value may be either a JSON object or a pre-serialized JSON string. A player is only scheduled if they have an entry here; any player in `user_ids` omitted from `custom_data` is reported in `errors.user_ids` and not scheduled.
| system_data
