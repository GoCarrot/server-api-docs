...
Description: The `v2/schedule` endpoint allows for bulk scheduling of notifications to be delivered within 30 days. Before using this API you must configure a corresponding xref:usage::page$scheduling.adoc#_local[Local Schedule, window=_blank] on the https://app.teak.io/login[Teak Dashboard, window=_blank]. Doing so gives you the full benefit of Teak’s analytics, A/B testing, and Content Management System when using local notifications.
...
| A map of Game Assigned Player IDs to a map of string key/value pairs. The key/value pairs will be available to be templated into the sent notification. See xref:usage::page$custom-tags.adoc#_local_notification_tags[Local Notification Tags, window=_blank] for more information.
|===
