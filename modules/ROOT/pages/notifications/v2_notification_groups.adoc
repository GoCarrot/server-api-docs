...
| Your Teak Server Secret
| `identifier`
| A unique identifier for the `<<type_notification_group,NotificationGroup>>` you are creating.
| `opt_out_category`
| The unique identifier of the opt-out category the `<<type_notification_group,NotificationGroup>>` will belong to.
| `channels`
...
! `'ok'`
! `notification_group`
! A `<<type_notification_group,NotificationGroup>>` dictionary.
!===
| Example
| [source, json, subs="attributes"]
----
...
Description: The `{endpoint}` endpoint allows for updating the `<<type_notification_group,NotificationGroup>>` with the identifier `identifier` in the request path.

**Note: For the time being, only a `<<type_notification_group,NotificationGroup>>` in which no `<<type_channel_group,ChannelGroup>>` contains more than one `<<type_notification_instance,NotificationInstance>>`.**
...
`NotificationInstance` represents the configuration of a notification, for a given channel.  If an A/B test is running, there may be multiple `NotificationInstance` records for a single channel, representing the variations under test.  It is a dictionary containing the following keys:
...
| `title`
| `string?`
| xref:ROOT:user-guide:page$notifications.adoc#_anatomy[See anatomy of a push notification docs.]
