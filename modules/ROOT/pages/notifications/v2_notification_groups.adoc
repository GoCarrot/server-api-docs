...
`EmailContentMode` controls how `message` is interpreted, when platform is `email`.  It is an enum comprised of one of the following values:
...
| `platforms`
| `Map<<<type_platform,Platform>>, <<type_platform_group,PlatformGroup>>>`
...
`NotificationInstance` represents the configuration of a notification, for a given platform.  If an A/B test is running, there may be multiple `NotificationInstance`s for a single platform, representing the variations under test.  It is a dictionary containing the following keys:
...
| `identifier`
| `string`
| The unique identifier of the `<<type_notification_group,NotificationGroup>>` to which this `PlatformGroup` belongs.

| `is_ab_test`
| `boolean`
| Whether or not this `PlatformGroup` has an active A/B test.  This will be true if there are multiple <<type_notification_instance,NotificationInstance>>s.
...
| `created_at`
| `string`
| The ISO8601 timestamp indicating when this `PlatformGroup` was originally created.

| `updated_at`
| `string`
| The ISO8601 timestamp indicating when this `PlatformGroup` was last updated.
|===
...
| ... _(Only available when platform is `email`.)_

| `android_content_mode`
| `<<type_android_content_mode,AndroidContentMode>>?`
| ... _(Only available when platform is `android`.)_
...
| `boolean`
| Displays the android banner above the expanded view image when the notification is expanded.  _(Only available when platform is `android`.  Must be false when `android_content_mode` is `ios_style`.)_
